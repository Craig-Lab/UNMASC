...
function [fit_parameters,resnorm, residual,exitflag,output,iterationData] = ...
    fit_surface(p,data,results)
...
iterationData = struct('Iteration', [], 'ResNorm', []);
...
options = optimoptions(@lsqnonlin,'Algorithm','trust-region-reflective', ...
    ...
    1e-6,'display','off','OutputFcn',@myOutputFunction); 

if p.monotonic == 0
    %Define parameter guess
    p.C11 = results{"C1",p.drug1_name};
    p.C12 = results{"C2",p.drug1_name};
    p.C21 = results{"C1",p.drug2_name};
    p.C22 = results{"C2",p.drug2_name};
    
    p.h11 = -results{"h1",p.drug1_name};
    p.h12 = -results{"h2",p.drug1_name};
    p.h21 = -results{"h1",p.drug2_name};
    p.h22 = -results{"h2",p.drug2_name};
...
% Nested function to capture iteration data
    function stop = myOutputFunction(~, optimValues, state)
        ...
        if strcmp(state, 'iter') || strcmp(state, 'done')
            ...
            iterationData.Iteration(end+1) = optimValues.iteration;
            iterationData.ResNorm(end+1) = optimValues.resnorm; 
            % iterationData.StepNorm(end+1) = norm(optimValues.step);
            % iterationData.FirstOrderOpt(end+1) = optimValues.firstorderopt;
        end
    end
end 
