function [] = fun_Dist_SingleFits(p, noiseParam, singleFits, combinationFits)
    ...
    fileTag = string(p.cell) + "_" + string(p.interaction) + "_" + string(p.vial);
    ...
    plotInfo(end+1,:) = {"C11", p.drug1, drug1_C1_col, ...
        "C1 lb", "C1 ub", "C1", "C11", false, "$\log_{10}(EC_{50,11})$", true};

    if p.drug1_mono == 0
        plotInfo(end+1,:) = {"C12", p.drug1, drug1_C2_col, ...
            "C2 lb", "C2 ub", "C2", "C12", false, "$\log_{10}(EC_{50,12})$", true};
    end

    plotInfo(end+1,:) = {"C21", p.drug2, drug2_C1_col, ...
        "C1 lb", "C1 ub", "C1", "C21", false, "$\log_{10}(EC_{50,21})$", true};

    if p.drug2_mono == 0
        plotInfo(end+1,:) = {"C22", p.drug2, drug2_C2_col, ...
            "C2 lb", "C2 ub", "C2", "C22", false, "$\log_{10}(EC_{50,22})$", true};
    end

    flip_h11 = p.drug1_mono == 1;
    flip_h21 = p.drug2_mono == 1;
    ...
    titleText = replace(string(p.interaction), "_", "-");
    ...
    for k = 1:nParams
        ...
        sampleMat = noiseParam.(p.vial).(p.interaction).(p.cell).(drugName);

        if sampleCol > size(sampleMat, 2)
            error("For %s %s %s %s: sample column %d does not exist. Sample matrix has only %d columns.", ...
                string(p.cell), string(p.vial), string(p.interaction), string(paramName), ...
        ...
        singleCol = matlab.lang.makeValidName( ...
            string(p.interaction) + "_" + string(p.vial) + "_" + string(drugName));

        if ~ismember(singleCol, string(singleFits.(p.cell).Properties.VariableNames))
            error("Could not find single-fit column %s in singleFits.%s.", ...
                singleCol, string(p.cell));
        end

        lb = singleFits.(p.cell){lbRow, singleCol};
        ub = singleFits.(p.cell){ubRow, singleCol};
        singleVal = singleFits.(p.cell){singleRow, singleCol};

        combCol = matlab.lang.makeValidName( ...
            string(p.interaction) + "_" + string(p.vial));

        if ~ismember(combCol, string(combinationFits.(p.cell).Properties.VariableNames))
            error("Could not find combination-fit column %s in combinationFits.%s.", ...
                combCol, string(p.cell));
        end

        combValRaw = combinationFits.(p.cell){combRow, combCol};
        combVal = combValRaw;
