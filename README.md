Code to run the model described in Bistodeau-Gagnon, Segal, Craig, and Michaels. 

### System requirements
All of the code was written and tested using MATLAB R2025b. The following toolboxes are required:  
...
Cells types for iPSCs are denoted as follows:  
	PROT: ProT cells  
	CD4ISP: CD4 single positive cells  
	CD3N: Double positive CD3- cells  
	CD3P: Double positive CD3+ cells  
	SP: CD8 single positive cells  
For the cord blood-derived cells are denoted as follows:
	CD7pCD5n: CD7+CD5- cells
	CD7pCD5p: CD7+CD5+ cells
For the cord blood-derived cells, the different donors are noted as "vials". In each folder for figures and supplementary figures, run file containing "commands" in the title to execute the code. 
When necessary, a folder “Figures” is included in the folder to store results files (.m files) and figures (in PDF). 
